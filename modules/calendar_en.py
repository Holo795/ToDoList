...
class Calendar:
    ...
    def __init__(self) -> None:
        ...
        self.months = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre",
                       "Novembre", "Décembre"]
        ...
        self.calendar[self.current_time["year"]][self.months[self.current_time["month"] - 1]][self.current_time["day"]][
            2] = "active"
    ...
    def set_days(self, nb_days_per_month: int, month, year):
        ...
        for day in range(1, nb_days_per_month + 1):
            var_temp.append({"nb_jour": day, "jour": self.days[self.days_loop], "type": "inactive",
                             "nb_jour_semaine": self.days_loop, "month": month, "year": year})
            self.days_loop += 1
            if self.days_loop == 7:
                self.days_loop = 0
        return var_temp
    ...
    def get_current_day(self, month_in_str=False) -> dict:
        ...
        if month_in_str:
            ...
            return {"year": dt.datetime.now().year, "month": months[dt.datetime.now().month],
